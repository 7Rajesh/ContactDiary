...
def edit_function(contacts):
    with open("ALL_Contacts.txt",mode='r') as file:         #creating dictionary by reading a text file
        for line in file:
            x=line.split(",")
            a=x[0]
            b=x[1]        
            ...
            if var in contacts.keys():                         #checking wheater contact exits or not
                ...
                while True :
                    ...
                    if len(inp_num) == 10:
                        ...
                        print(contacts)
                        with open('ALL_Contacts.txt', 'w') as file:     #if update occur then writng the update one         
